...
public class DeleteRecipeInvalidTokenTests : BaseIntegrationTest
{
    private const string REQUEST_URI = "recipes";

    private readonly string _tokenUserNotExistDatabase;

    public DeleteRecipeInvalidTokenTests(MyRecipeBookApplicationFactory factory) : base(factory)
    {
    ...
    {
        var response = await Delete($"{REQUEST_URI}/{Guid.CreateVersion7()}", accessToken: "tokenInvalid");
    ...
    {
        var response = await Delete($"{REQUEST_URI}/{Guid.CreateVersion7()}", accessToken: string.Empty);
    ...
    {
        var response = await Delete($"{REQUEST_URI}/{Guid.CreateVersion7()}", accessToken: _tokenUserNotExistDatabase);
